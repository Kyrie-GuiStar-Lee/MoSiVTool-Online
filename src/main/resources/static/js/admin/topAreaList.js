...
ul.eq(4).children().eq(2).children().eq(0).addClass("active");

// 设置数据
function setContentData() {
    $.ajax({
        type: "POST",
        url: "/EasyPlatform/areaManage/getTopAreaList",
        dataType: "json",
        data: {
...
$("#addArea").on("click", function () {
    layer.open({
        ...
        area: ['600px', '550px'],
        content: '/EasyPlatform/areaManage/addTopAreaPage',
        end: function () {
            setContentData();
        }
    });
...
function updateArea(areaId) {
    layer.open({
        ...
        shadeClose: true, //点击遮罩关闭层
        area: ['600px', '550px'],
        content: '/EasyPlatform/areaManage/updateTopAreaPage?areaId=' + areaId,
        end: function () {
            setContentData();
        }
    });
