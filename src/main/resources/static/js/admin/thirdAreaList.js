...
ul.eq(4).children().eq(2).children().eq(2).addClass("active");

// 设置数据
function setContentData() {
    $.ajax({
        type: "POST",
        url: "/EasyPlatform/areaManage/getThirdAreaList",
        dataType: "json",
        data: {
...
function updateArea(areaId) {
    layer.open({
        ...
        area: ['600px', '550px'],
        content: '/EasyPlatform/areaManage/updateThirdAreaPage?areaId=' + areaId,
        end: function () {
            setContentData();
        }
    });
...
$("#addArea").on("click", function () {
    layer.open({
        ...
        shadeClose: true, //点击遮罩关闭层
        area: ['600px', '550px'],
        content: '/EasyPlatform/areaManage/addThirdAreaPage',
        end: function () {
            setContentData();
        }
    });
